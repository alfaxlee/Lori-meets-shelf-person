...
新登場 Boss「大猩猩」（無敵大猩猩），初始血量 200，進入戰鬥後展開連續不間斷的**瘋狂跳躍攻擊**，每次落地均會產生如同蘿莉跳躍時的強力雙向擴散震波。

#### 核心特性

- **血量**：200
- **行為模式**：**瘋狂跳躍**（300ms 快速預警 → 750ms 超高空 550~750px 巨幅拋物線翻滾躍擊（全圖多模式高度隨機落點） → 落地強震 → 80~150ms 超短間隔後立刻進行下一次連跳，形成狂暴高頻瘋狂跳躍）
- **落地震波**：落地時引發鏡頭震動，並向左、右兩側高速發射 260x120 且擴散 2.2 倍的超巨型土褐色地表衝擊波（shockwaves），玩家觸碰即死
