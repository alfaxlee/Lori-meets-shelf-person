...
- [我沒有GG Boss（HP: 500）](#我沒有gg-bosshp-500)
- [武器系統](#武器系統)
- [特殊機制](#特殊機制)
- [技術棧](#技術棧)
- [專案架構](#專案架構)
...
<!-- 新增中文註解：在 README.md 中最新更新我沒有GG Boss 說明 -->
### 我沒有GG Boss（HP: 500）

Boss「我沒有GG」登場於畫面中央偏高處，具備地心引力判定，平時保持靜止，並會定期向玩家發射「迪克小刀」彈幕。

#### 運作與攻擊機制

- **移動與地心引力 AI**
  - **地心引力判定**：具備重力加速度（`allowGravity = true`），開局或重生時會從空中向下滑落並降落在地板上。
  - **受擊擊退與自然阻力**：平時無位移追逐 AI。當被玩家子彈或衝刺護盾擊中時，會被向斜後方彈飛擊退，並在重力與阻力作用下自然墜落回地面並停下。

- **攻擊招式**
  - **迪克小刀**（每 2.5 ~ 3.5 秒）：向玩家當前位置精準發射一枚高速飛行（速度 550）的「迪克小刀」（帶有紅光提示，無視重力直線飛行）。
  - **碰撞與阻擋**：迪克小刀碰到玩家會引發當機即死（`triggerCrash`）；碰到地板或玩家的衝刺護盾（Dash Shield）會被成功阻擋並銷毀。

#### HP 閾值事件

| HP | 事件 |
|----|------|
| 500 | 初始狀態，受到重力自然下墜，開始定時發射迪克小刀 |
| ≤ 0 | Boss 死亡 → 3 秒後於中央高空重生 |

#### 受擊機制

| 項目 | 說明 |
|------|------|
| 受擊效果 | 受到傷害時會閃紅、被擊退彈飛，隨後受地心引力自然墜落回地面停下 |
