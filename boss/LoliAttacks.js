...
export function scheduleNextLaser(scene) {
    const loli = state.loli;
    if (loli.isBerserk || !loli.active || loli.isSuperInvincible) return;
    const delay = Phaser.Math.Between(3000, 7000);
    scene.time.delayedCall(delay, () => {
        if (loli.active && !loli.isBerserk && !loli.isSuperInvincible) {
            spawnLaser(scene);
...
export function clearAllAttacks(scene) {
    if (state.shockwaves) state.shockwaves.clear(true, true);
    if (state.lasers) state.lasers.clear(true, true);
    if (state.enemyBalls) state.enemyBalls.clear(true, true);
    // 清除所有警告線
